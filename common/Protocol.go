...
//定义一个执行状态的结构,任务在执行的时候会把状态记录在这里边
type JobExecuteInfo struct {
	Job        *Job               //任务信息
	PlanTime   time.Time          //理论上的任务调度开始时间
	RealTime   time.Time          //实际的调度时间
	CancelCtx  context.Context    //任务command的context
	CancelFunc context.CancelFunc //用于取消command执行的cancel函数
	//CancelCtx和CancelFunc总是一起出现的
}
...
type JobLog struct {
	JobName      string `json:"jobName" bson:"jobName"`           //任务名字 分别是json和bson的序列化方式
...
type LogBatch struct {
	//每行日志放到一个interface{}中，多行日志放到一个数组中
	Logs []interface {
	}
}
...
func BuildResponse(errno int, msg string, data interface{}) (resp []byte, err error) {
	...
	response.Data = data
	//序列化json
	resp, err = json.Marshal(response) //结构体变成了二进制json
	fmt.Println("BuildResponse", string(resp))
	return
}
...
func BuildJobExecuteInfo(jobSchedulePlan *JobSchedulePlan) (jobExcuteInfo *JobExecuteInfo) {
	jobExcuteInfo = &JobExecuteInfo{
		...
		PlanTime: jobSchedulePlan.NextTime, //理论上的调度时间
...
type SortLogByStartTime struct {
	SortOrder int `bson:"startTime"` //(startTime: -1) -1表示降序排列
}
